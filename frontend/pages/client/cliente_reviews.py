...
from apiCalls.client.apiClientReviews import (
    fetch_reviews,
    fetch_reviews_with_filters,
    create_review,
    create_many_reviews,
    update_review,
    update_many_reviews_by_ids,
    delete_review,
    delete_many_reviews_by_ids,
)

from apiCalls.client.apiClientUsers import fetch_user_by_id
from apiCalls.client.apiClientRestaurants import fetch_restaurant_by_id

def cliente_reviews_page():
    st.header("Dejar o ver Reseñas")
    st.markdown("Aquí puedes dejar reseñas sobre los restaurantes que has visitado. También puedes ver todas las reseñas que has dejado y filtrarlas por diferentes criterios.")
    ...
    with tabs[0]:
        ...
        reviews = fetch_reviews(user_id=user_id)
        per_page = 10
        total = len(reviews)
        ...
        if total == 0:
            ...
            for rev in slice_reviews:
                ...
                st.markdown(f"**ID de la Reseña:** {rev['_id']}")
    ...
    with tabs[1]:
        ...
        uid = st.session_state.user["_id"]
        st.text_input("User ID", value=uid, disabled=True, key="filter_uid_display")
    ...
    with tabs[2]:
        ...
        c_user = st.session_state.user["_id"]
        st.text_input("User ID", value=c_user, disabled=True, key="create_user_display")
        if not c_user:
